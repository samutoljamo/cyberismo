...
def _emit_solver_stats_ql_table_typst(
    ...
    # Wider gutter between the two project halves so numbers in adjacent
    # cells do not visually run together; tighter within each base./+rf pair.
    gutters: list[str] = []
    cols_per_project = len(_SOLVER_STATS_SCALES) * n_variant_cols
    for i in range(total_cols - 1):
        # i is the index of the column to the LEFT of this gutter.
        if i == 0:
            # Between label and first data column. Needs extra room because
            # the label column auto-sizes to the widest label ("Equivalences"),
            # so this gutter is the entire visual gap for that row.
            gutters.append("2em")
        elif i == cols_per_project:
            # Between the two projects.
            gutters.append("1.4em")
        elif (i - 1) % n_variant_cols == n_variant_cols - 1:
            # Between scales within a project (after each base./+rf pair).
            gutters.append("0.9em")
        else:
            # Within a base./+rf pair.
            gutters.append("0.3em")
    column_gutter_tuple = "(" + ", ".join(gutters) + ")"

    project_vline_col = 1 + cols_per_project
    ...
    lines: list[str] = [
        ...
        f"    column-gutter: {column_gutter_tuple},",
        f"    align: {align_tuple},",
        "    stroke: none,",
        "    table.hline(),",
    ...
    # Pad label cells so the first data cell aligns visually in the source.
    label_cell_width = max(len(f"[{label}],") for label, _ in rendered_rows) if rendered_rows else 0
    for label, values in rendered_rows:
        formatted = ", ".join(f"[{_format_thousands_typst(v)}]" for v in values)
        prefix = f"[{label}],".ljust(label_cell_width)
        lines.append(f"    {prefix} {formatted},")

    lines.append("    table.hline(),")
    lines.append(f"    table.vline(x: {project_vline_col}, stroke: 0.3pt),")
    lines.append("  )")
    lines.append("}")
